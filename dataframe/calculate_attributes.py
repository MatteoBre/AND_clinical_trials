...
def get_all_jds_sts_similarities(clinical_trials, articles, max_jds, max_sts, mode='basic'):
    ...
    for i in range(len(clinical_trials)):
        ...
        if mode == 'basic':
            jds_similarity = get_jds_sts_basic_similarities(java_server.get_jds(ct_texts)[:max_jds],
                                                            java_server.get_jds(ar_texts)[:max_jds])
        if mode == 'percentage':
            jds_similarity = get_jds_sts_percentage_similarities(java_server.get_jds(ct_texts)[:max_jds],
                                                                 java_server.get_jds(ar_texts)[:max_jds])
        if mode == 'ranking':
            jds_similarity = get_jds_sts_ranking_similarities(java_server.get_jds(ct_texts)[:max_jds],
                                                              java_server.get_jds(ar_texts)[:max_jds])
        if mode == 'percentage_ranking':
            jds_similarity = get_jds_sts_percentage_ranking_similarities(java_server.get_jds(ct_texts)[:max_jds],
                                                                         java_server.get_jds(ar_texts)[:max_jds])
        jds_similarities.append(jds_similarity)

        if mode == 'basic':
            sts_similarity = get_jds_sts_basic_similarities(java_server.get_sts(ct_texts)[:max_sts],
                                                            java_server.get_sts(ar_texts)[:max_sts])
        if mode == 'percentage':
            sts_similarity = get_jds_sts_percentage_similarities(java_server.get_sts(ct_texts)[:max_sts],
                                                                 java_server.get_sts(ar_texts)[:max_sts])
        if mode == 'ranking':
            sts_similarity = get_jds_sts_ranking_similarities(java_server.get_sts(ct_texts)[:max_sts],
                                                              java_server.get_sts(ar_texts)[:max_sts])
        if mode == 'percentage_ranking':
            sts_similarity = get_jds_sts_percentage_ranking_similarities(java_server.get_sts(ct_texts)[:max_sts],
                                                                         java_server.get_sts(ar_texts)[:max_sts])
